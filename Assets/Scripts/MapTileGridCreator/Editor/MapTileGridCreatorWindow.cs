...
public class MapTileGridCreatorWindow : EditorWindow
{
	...
	//Selection
	private List<MapTileGridCreator.Core.Cell> _selection = new List<MapTileGridCreator.Core.Cell>();
	...
	{
        MapTileGridCreator.Core.Cell[] selection = Selection.GetFiltered<MapTileGridCreator.Core.Cell>(SelectionMode.TopLevel);
		ReplaceSelection(selection);
	}

	private void ReplaceSelection(MapTileGridCreator.Core.Cell[] selection)
	{
		_selection.Clear();
		foreach (MapTileGridCreator.Core.Cell c in selection)
		{
            _selection.Add(c);
		}
	...
	private Vector3 GetGridPositionInput(float offset_normal_factor, bool canCollideWithPlane)
	{
		...
		{
			hitPoint = hit.point;
			if (hit.collider.gameObject.GetComponent<MapTileGridCreator.Core.Cell>() != null || hit.collider.GetComponentInParent<MapTileGridCreator.Core.Cell>())
			{
			...
			case SelectMode.Stamp:
				...
				if (GUILayout.Button("Load from prefab"))
				{
					...
					if (fullpath != "")
					{
                        ...
                        ReplaceSelection(prefab.GetComponentsInChildren<MapTileGridCreator.Core.Cell>());
					}
				...
				if (_selection.Count != 0 && GUILayout.Button("Save to prefab"))
				{
					...
					if (fullpath != "")
					{
						...
						foreach (MapTileGridCreator.Core.Cell c in _selection)
						{
                            ...
                            UnityEditorInternal.ComponentUtility.PasteComponentValues(child.GetComponent<MapTileGridCreator.Core.Cell>());
						}
