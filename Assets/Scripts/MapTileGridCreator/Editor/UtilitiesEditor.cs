	...
	public static class FuncEditor
	{
		...
		private static void DebugSquareGrid(Grid3D grid, Color color, int offset_grid_y = 0, int size_grid = 10)
		{
			using (new Handles.DrawingScope(color))
			{
				...
				{
					Handles.DrawLine(pos + new Vector3(-.5f, 0, z + 0.5f) * CaseSize,
									pos + new Vector3(size_grid + .5f, 0, z + 0.5f) * CaseSize);
				}

				for (float x = -1; x <= size_grid; x++)
				{
					Handles.DrawLine(pos + new Vector3(x + 0.5f, 0, -.5f) * CaseSize,
									pos + new Vector3(x + 0.5f, 0, size_grid + .5f) * CaseSize);
				}
			}
		}
		...
		public static Core.Cell InstantiateCell(GameObject prefab, Grid3D grid, Vector3 position)
		{
